...
$propics=md5($img).$extension;
     ...
     $query=$pdoConnection-> query("update tblartist set Profilepic ='$propics' where ID='$imgid'");
    if ($query) {
      echo "<script>alert('Artist profile pic has been updated.');  location.href='edit-artist-detail.php?editid=$imgid'</script>";
...
<head>
  ...
  <title>Update Trainee Photo | Peace Sports School Admission System</title>
...
<body>
  ...
  <?php
 $imgid=$_GET['imageid'];
$query=$pdoConnection-> query("select * from tblartist where ID='$imgid'");
$cnt=1;
